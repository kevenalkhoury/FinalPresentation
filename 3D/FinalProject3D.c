...
//We solve the Navier–Stokes equations on an adaptive octree.#include "grid/octree.h"
#include "navier-stokes/centered.h"

//We will use the λ2 criterion of Jeong and Hussain, 1995 for vortex detection.

#include "lambda2.h"

//This is the maximum level of refinement i.e. an equivalent maximum resolution of 2563.

int maxlevel = 8;

//The domain size is 163. We move the origin so that the center of the unit sphere is not too close to boundaries.
...
//We use gfsview and ppm2gif to create the animated isosurface of λ2 for 30<=t<=60.

event movies (t = 30; t += 0.25; t <= 60) {
  static FILE * fp =
    popen ("gfsview-batch3D sphere.gfv | ppm2gif > sphere.gif", "w");

//Here we compute two new fields, λ2 and the vorticity component in the y−z plane.
